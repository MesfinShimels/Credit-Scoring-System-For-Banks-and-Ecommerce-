...
class RFMSRiskClassifier:
    ...
    def calculate_woe_iv(self, df, feature, target):
        ...
        - target: Binary column name representing the target variable (1 for positive, 0 for negative).
        ...
        try:
            ...
            if not set(df[target].unique()).issubset({0, 1}):
                raise ValueError(f"Target column '{target}' must be binary (contain only 0 and 1).")
            ...
            if total_events == 0 or total_non_events == 0:
                raise ValueError(f"Feature '{feature}' has no events or non-events for target '{target}'.")
            ...
            grouped['events_ratio'] = grouped['events'] / total_events
            grouped['non_events_ratio'] = grouped['non_events'] / total_non_events

            # Replace zero ratios with a small value to avoid log issues
            grouped['events_ratio'].replace(0, 1e-10, inplace=True)
            grouped['non_events_ratio'].replace(0, 1e-10, inplace=True)

            # Calculate WoE and IV
            grouped['woe'] = np.log(grouped['events_ratio'] / grouped['non_events_ratio'])
            grouped['iv'] = (grouped['events_ratio'] - grouped['non_events_ratio']) * grouped['woe']

            # Total IV for the feature
            total_iv = grouped['iv'].sum()

            # Store the IV in a DataFrame
