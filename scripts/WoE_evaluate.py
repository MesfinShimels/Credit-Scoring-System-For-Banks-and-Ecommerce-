    ...
    def woe_num(self):
        ...
        for col in numerical_features:
            if col not in self.df.columns:
                logging.warning(f"Column {col} not found in the dataset. Skipping...")
                continue

            # Check for NaN or infinite values
            if self.df[col].isnull().any() or np.isinf(self.df[col]).any():
                logging.warning(f"Column {col} contains NaN or infinite values. Please clean your data.")
                continue

            try:
                # Decision Tree for monotonic binning
                x = self.df[col].values.reshape(-1, 1)
                y = self.df[self.target]

                clf = DecisionTreeClassifier(max_leaf_nodes=5)  # Adjust max_leaf_nodes for finer/coarser bins
                clf.fit(x, y)

                # Get bin edges from DecisionTreeClassifier
                thresholds = np.sort(clf.tree_.threshold[clf.tree_.threshold > -2])
                self.breaks[col] = list(thresholds)

                logging.info(f"Binning successful for {col}, breaks: {self.breaks[col]}")
            except Exception as e:
                logging.error(f"Error in binning for {col}: {e}")
